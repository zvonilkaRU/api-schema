...
// Ответ POST /auth/email/confirm. Полный профиль + токены (подтверждение = вход).
type EmailConfirmResponseRequest struct {
...
type EmailConfirmResponseResponse struct {
	// Полный профиль пользователя. Возвращается ТОЛЬКО из /users/me (владельцу). login и email скрыты от других пользователей.
...
func (x EmailConfirmResponseRequest) ValidateOwn(reg *validator.Registry) error {
...
func (x EmailConfirmResponseResponse) ValidateOwn(reg *validator.Registry) error {
	if len(x.AccessToken) < 1 {
		return fmt.Errorf("field AccessToken: must be >= 1")
	}
	if len(x.RefreshToken) < 1 {
